...
namespace Telerik.Web.Mvc.Tests
{
    ...
    public class WebAssetLocatorTests
    {
        ...
        public void Locate_should_probe_all_extensions_in_debug_mode()
        {
            ...
            _virtualPathProvider.Setup(vpp => vpp.FileExists("~/scripts/1.0/jquery-1.3.2.min.js")).Returns(true);
            _virtualPathProvider.Setup(vpp => vpp.FileExists("~/scripts/jquery-1.3.2.js")).Returns(true);
        ...
        {
            _locator = new WebAssetLocator(false, _virtualPathProvider.Object);
            Assert.Equal("http://www.example.com", _locator.Locate("http://www.example.com", null));
        }
    }
}