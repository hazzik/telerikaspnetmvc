...
namespace Telerik.Web.Mvc.Examples.Models
{
    public static class SessionClientOrderRepository
    {
        public static IEnumerable<ClientEditableOrder> All()
        {
            ...
            if (result == null)
            {
                HttpContext.Current.Session["clietOrders"] = result = new NorthwindDataContext().Orders.Select(o => new ClientEditableOrder { OrderID = o.OrderID, OrderDate = o.OrderDate ?? DateTime.Now, Employee = o.Employee.FirstName + " " + o.Employee.LastName, Freight = o.Freight ?? 0 }).ToList();
            }
